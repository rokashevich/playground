...
class session {
 ...
 public:
  static void serve(std::shared_ptr<session> pThis, io_service& ios) {
    std::cout << "remote ip=" << pThis->socket.remote_endpoint().address()
              << std::endl;
    // К нам в сокет приходит GET запрос который по спецификации имеет следующий
    // вид: "GET /script.sh HTTP/1.1\r\n ..." Т.е. когда в бразуере
    // пишешь 127.0.0.1:8080/script.sh то отправляется запрос именно
    // такого вида. И непосредственно создание асинхронной задачи,
    // функция serve по сути обёртка что бы не утяжелять функцию
    // accept_and_run.
    asio::async_read_until(
        pThis->socket, pThis->buff,
        '\r',  // читаем данные сокет до первого \r
        [pThis, &ios](const error_code& e, std::size_t s) {
          // Внутри callback-а, который вызывается, когда boost.asio
          // считал GET запрос до символа \r (то что дальше нам и не
          // надо).
          ...
          // Исходное состояние: скрипт не запущен, количество
          // подключенных сессий ноль. Как только появляется первый
          // коннект (одновременно двух не может быть т.к. асинхронщина
          // это не многопоточность, а один поток!) - запускаем скрипт
          // (асинхронную задачу с колбэком) и в любом случае добавляем
          // идентификатор обрабатываемой сессии в статический массив
          // sessions. По завершении скрипта выполняется callback который
          // в каждую сохранённую сессию пишет код возврата 200 ОК, что
          // скрипт завершился, и обнуляем массив сессий - состояние
          ...
          if (session::sessions.empty()) {
            pThis->c = boost::process::child(
                ...
                boost::process::on_exit([](int e, const std::error_code& ec) {
                  // Внутри callback после завершения процесса. Вывод
                  // скрипта игнорируем, e - код возврата. Итерируемся по
                  // всем сессиям, которые подключились в процессе
                  // выполнения скрипта и каждому отправляем заголовок о
                  // завершении.
